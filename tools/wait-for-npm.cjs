...
function updateBody(endpoint, body) {
  ghApi(endpoint, { method: 'PATCH', input: JSON.stringify({ body }) })
}
...
async function isPublished(version) {
  const response = await fetch(
    ...
    { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(REQUEST_TIMEOUT) },
  )
...
module.exports = { addPendingNotice, discussionEndpoint, removePendingNotice }
