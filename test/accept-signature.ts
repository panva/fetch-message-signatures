...
describe('Accept-Signature creation and parsing', () => {
  ...
  it('parses one or more spaces after parameter delimiters', () => {
    assert.deepEqual(
      parseAcceptSignature(
        'sig1=("content-type"; sf "cache-control";   sf); created;   alg="future-example-alg"',
      ),
      [
        {
          label: 'sig1',
  ...
  it('rejects malformed members, duplicate labels, and duplicate components', () => {
    ...
    assert.throws(() => parseAcceptSignature('sig1=@1659578233'), /must be an Inner List/)
    assert.throws(() => parseAcceptSignature('sig1=%"display"'), /must be an Inner List/)
    assert.throws(
      () => parseAcceptSignature('sig1=(@1659578233)'),
      /Covered component identifiers must be Structured Field Strings/,
    )
    assert.throws(
      () => parseAcceptSignature('sig1=(%"display")'),
      /Covered component identifiers must be Structured Field Strings/,
    )
    assert.throws(
      () => parseAcceptSignature('sig1=("@status");created=@1659578233'),
      /Requested signature parameter "created" must be a bare Boolean true/,
    )
    assert.throws(
      () => parseAcceptSignature('sig1=("@status");alg=%"ed25519"'),
