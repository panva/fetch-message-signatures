...
describe('signature field parsing and pairing', () => {
  it('parses ordered components, parameters, tokens, and byte sequences', () => {
    assert.deepEqual(
      parseSignatureInput(
        'sig1=("@method" "content-type";sf);created=1618884473;alg="hmac-sha256";custom=thing;decimal=1.0',
      ),
      [
        {
          label: 'sig1',
    ...
    assert.deepEqual(parseSignature('sig1=:AAEC:'), [
...
describe('multiple signatures', () => {
  ...
  it('reuses the response body stream without tee buffering', async () => {
    ...
    await assert.rejects(response.text(), /Body is unusable/)
  })

  it('transfers the request body without tee buffering', async () => {
...
describe('synchronous cryptographic adapters', () => {
  it('uses synchronous primitives behind Promise-based provider methods', async () => {
    let signerFactoryCalls = 0
    let verifierFactoryCalls = 0
    const signerProvider = syncCryptoSigner()
    const verifierProvider = syncCryptoVerifier(undefined, 'sync-key')
    ...
    const signed = await sign(rfcRequest(), {
      ...
      parameters: { created: RFC_CREATED, keyid: 'sync-key', alg: 'hmac-sha256' },
      label: 'sync',
    })

    const result = await verify(signed, {
      verifier(signature, context) {
        verifierFactoryCalls++
        return verifierProvider(signature, context)
      },
      policy: verificationPolicy({
        requiredComponents: ['@method', '@target-uri', 'date'],
        requiredParameters: ['created', 'keyid', 'alg'],
      }),
    })

    assert.equal(result.label, 'sync')
    assert.equal(signerFactoryCalls, 1)
    assert.equal(verifierFactoryCalls, 1)
  })

  it('copies Buffer outputs returned by Node.js cryptography', async () => {
    let output!: Buffer
    const fields = await createSignature(rfcRequest(), {
      signer: () => ({
        type: 'signer',
...
describe('signed fetch', () => {
  it('uses manual redirects so redirected requests can be re-signed by the application', async () => {
