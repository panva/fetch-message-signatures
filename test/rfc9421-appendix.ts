...
import { RFC_CREATED, rfcRequest, rfcResponse, verificationPolicy } from './support.ts'

const rsaPssPublicKey = createPublicKey(`-----BEGIN PUBLIC KEY-----
...
const p256PublicKey = createPublicKey(`-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEqIVYZVLCrPZHGHjP17CTW0/+D9Lf
w0EkjqF7xB4FivAxzic30tMM4GF+hR6Dxh71Z50VGGdldkkDXZCnTNnoXQ==
-----END PUBLIC KEY-----`)

const ed25519PublicKey = createPublicKey(`-----BEGIN PUBLIC KEY-----
MCowBQYDK2VwAyEAJrQLj5P/89iXES9+vFgrIy29clF9CC/oPPsw3c5D0bs=
-----END PUBLIC KEY-----`)

const ed25519PrivateKey = createPrivateKey(`-----BEGIN PRIVATE KEY-----
...
const rsaPssKey = await crypto.subtle.importKey(
  'spki',
  new Uint8Array(rsaPssPublicKey.export({ type: 'spki', format: 'der' })),
...
const p256Verifier = ecdsaP256Sha256Verifier(
  await crypto.subtle.importKey(
    'spki',
    new Uint8Array(p256PublicKey.export({ type: 'spki', format: 'der' })),
...
const ed25519Verifier = createEd25519Verifier(
  await crypto.subtle.importKey(
    'spki',
    new Uint8Array(ed25519PublicKey.export({ type: 'spki', format: 'der' })),
    'Ed25519',
    false,
    ['verify'],
  ),
...
const ed25519Signer = createEd25519Signer(
  await crypto.subtle.importKey(
    'pkcs8',
    new Uint8Array(ed25519PrivateKey.export({ type: 'pkcs8', format: 'der' })),
    'Ed25519',
    false,
    ['sign'],
  ),
...
describe('RFC 9421 Appendix B complete examples', () => {
  ...
  it('creates the exact deterministic B.2.5 HMAC-SHA256 signature', async () => {
    ...
    assert.equal(Buffer.from(fields.signature).toString('base64'), b25Signature)
    assert.equal(
  ...
  it('creates the exact deterministic B.2.6 Ed25519 signature', async () => {
    ...
    assert.equal(Buffer.from(fields.signature).toString('base64'), b26Signature)
    assert.equal(
      fields.signatureInput,
...
describe('RFC 9421 Appendix B.3 TLS-terminating proxy', () => {
  ...
  function proxyRequest(): Request {
    return new Request('https://service.internal.example/foo?param=Value&Pet=dog', {
      method: 'POST',
      headers: {
        date: 'Tue, 20 Apr 2021 02:07:55 GMT',
        'content-type': 'application/json',
