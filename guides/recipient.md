...
1. parse the signature and enforce an application policy;
2. select trusted key material and an algorithm; and
3. verify the reconstructed signature base.

`verify()` requires all three. It does not expose a mode that accepts any cryptographically valid
signature.

## Resolve a trusted verifier

The verifier factory receives parsed, unauthenticated metadata. Treat every value—including `keyid`,
`alg`, `tag`, and the covered component list—as attacker-controlled until verification succeeds.
...
For a selected label, `verify()`:

1. parses and pairs the `Signature-Input` and `Signature` dictionary members;
2. validates that covered components apply to the target message;
3. applies required coverage, metadata, algorithm-signal, and timestamp policy;
4. reconstructs and snapshots the signature base;
5. invokes the verifier factory;
6. confirms the trusted verifier algorithm against policy and any `alg` signal;
7. invokes cryptographic verification and rechecks time policy; and
8. invokes `policy.validate`, followed by a final time-policy check.

Keep expensive or state-changing application work after step 7.
