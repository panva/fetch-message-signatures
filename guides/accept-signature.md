...
`created` appears without a value: the request asks for a creation timestamp but the signer picks
it. `keyid="server-key"` carries a value, so the signer must use that exact key identifier.
