...
The helper deliberately requires explicit confirmation for parameters whose requested form is not
enough to safely choose a value:
...
## Request expiration safely

An `expires` request does not specify a duration. The signer must select a concrete time:

```ts
declare const response: Response
declare const relatedRequest: Request
declare const signatureRequest: FetchSig.SignatureRequest
declare const signer: FetchSig.SignerFactory
...
const fields = await FetchSig.createRequestedSignature(response, signatureRequest, {
  ...
  now,
  parameters: [['expires', now + 60]],
})
```

Choose the lifetime from local policy, not from untrusted context. The eventual recipient should
also enforce its own `maxAge`, expiration, and replay policy.
