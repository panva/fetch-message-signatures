...
Cloudflare Workers decodes received field values as UTF-8 rather than exposing them as byte strings.
A field carrying a non-ASCII octet therefore yields different code units there than on the other
runtimes, which changes both the combined value and the Byte Sequences produced by `bs`. The
received octets cannot be recovered from the decoded string, so supply `fieldValues` with the raw
octets when a non-ASCII field value has to be signed on Workers. ASCII field values, which is what
RFC 9110 recommends and what nearly all fields use, are unaffected.
...
Bun, and Workers accept it and would otherwise place the password in the signature base and in
anything that logs or exchanges it.
...
Node.js, Deno, and browsers preserve a lowercase method such as `patch`, while Bun and Workers
normalize it to `PATCH`, and Bun replaces an unrecognized method with `GET`. The method has already
been changed by the time this package sees it, so use the canonical uppercase spelling when a
signature has to reproduce across runtimes.
