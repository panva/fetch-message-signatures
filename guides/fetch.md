...
RFC 9421 models HTTP messages at the protocol layer. Fetch exposes a safer, higher-level view that
normalizes some values and hides others. Integrations must understand the resulting boundary.
...
The Request and Response helpers transfer or reuse the source body stream rather than teeing it.
This avoids unbounded buffering for long-lived or large streaming bodies. Do not read the source
message after constructing the signed message; consume the returned message instead.
...
reconstructed signed response preserves:
...
It cannot preserve Fetch-managed `url`, `redirected`, or `type` metadata. Opaque and error responses
have status zero and cannot be reconstructed by the append helpers.
...
`createSignedFetch()` changes `redirect: 'follow'` to `redirect: 'manual'`. To follow safely:
...
`signature`, `signature-input`, and `accept-signature` are known RFC 8941 Dictionaries. For an
application field, a string mapping (`'dictionary'`, `'list'`, or `'item'`) uses RFC 8941 grammar.
Use `{ type: '…', version: 'rfc9651' }` only when the field's defining specification uses RFC 9651;
this enables its Date and Display String item types.
...
Browser CORS processing can prevent script from sending or observing fields. Response types can also
hide status and headers. A signature cannot cover data that the runtime does not expose.

Plan for:

- CORS preflight for signature-related request fields;
- `Access-Control-Expose-Headers` for signature-related response fields;
- service worker or extension transformations;
- browser-managed fields that script cannot set; and
- opaque responses, which cannot be verified from page JavaScript.

## Signed Fetch behavior

`createSignedFetch()` accepts the normal Fetch input and init arguments. It constructs a request,
forces manual redirects when needed, signs it, invokes the configured Fetch implementation, and
optionally verifies the response against that exact signed request.

Its signing and verification configuration is copied when the wrapper is created, so mutating the
original options object cannot disable verification while a request is in flight. Keep changing key
material behind the provider factories or create a new wrapper when policy changes.

It returns the response unchanged after successful verification. It does not consume the response
body and does not validate a body digest.
