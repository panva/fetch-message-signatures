...
const HTTP_SIGNATURE_ALGORITHMS = new Set([
  'rsa-pss-sha512',
  'rsa-v1_5-sha256',
  'hmac-sha256',
  'ecdsa-p256-sha256',
...
/** The Structured Fields grammar used to parse and serialize an HTTP field. */
export type StructuredFieldVersion = 'rfc8941' | 'rfc9651'

/** The top-level type and grammar version of an HTTP Structured Field. */
export interface StructuredFieldDefinition {
  readonly type: StructuredFieldType
  readonly version: StructuredFieldVersion
}
...
export type SignatureParameterValue =
  string | number | boolean | Uint8Array | StructuredFieldToken | StructuredFieldDecimal
...
export interface SignatureContext {
  ...
  readonly structuredFields?: Readonly<
    Record<string, StructuredFieldType | StructuredFieldDefinition>
  >
...
interface ParseState {
  ...
  readonly extendedTypes: boolean
  index: number
...
function parseBareItem(state: ParseState): SfBareItem {
  ...
  if (character === '@' || character === '%') {
    if (!state.extendedTypes) {
      fail(
        'RFC 9651 item types are not part of RFC 8941 Structured Fields and are not valid RFC 9421 signature parameters',
      )
    }
...
function parseStructuredField(
  ...
  const state: ParseState = { input, extendedTypes, index: 0, duplicateKeys: [] }
  skipSp(state)
...
function signatureParameterFromSf(item: SfBareItem): SignatureParameterValue {
  switch (item.kind) {
    ...
    case 'display-string':
      return fail(
        'RFC 9651 Date and Display String values are not valid RFC 9421 signature parameters',
      )
  }
}
  ...
  for (const [name, value] of parameters) {
    switch (name) {
      ...
      case 'tag':
        ...
        if (name === 'alg' && !HTTP_SIGNATURE_ALGORITHMS.has(value.value)) {
          fail(`Signature parameter "alg" is not a registered HTTP signature algorithm`)
        }
        break
    }
  }
...
interface ResolvedStructuredFieldDefinition {
  readonly type: StructuredFieldType
  readonly version: StructuredFieldVersion
}

function structuredFieldDefinition(
  name: string,
  options: SignatureContext,
): ResolvedStructuredFieldDefinition | undefined {
  if (name === 'signature-input' || name === 'signature' || name === 'accept-signature') {
    return { type: 'dictionary', version: 'rfc8941' }
  }
  if (options.structuredFields !== undefined) {
    ...
    if (configured === 'dictionary' || configured === 'list' || configured === 'item') {
      return { type: configured, version: 'rfc8941' }
    }
    if (
      configured === null ||
      typeof configured !== 'object' ||
      (configured.type !== 'dictionary' && configured.type !== 'list' && configured.type !== 'item')
    ) {
      fail(`Structured Field type for "${name}" is invalid`)
    }
    if (configured.version !== 'rfc8941' && configured.version !== 'rfc9651') {
      fail(`Structured Field version for "${name}" is invalid`)
    }
    return { type: configured.type, version: configured.version }
  }
  return undefined
}
...
function fieldComponentValue(
  ...
  if (key !== undefined) {
    const definition = structuredFieldDefinition(componentValue.name, options)
    if (definition !== undefined && definition.type !== 'dictionary') {
      fail(
        `Structured Field type for "${componentValue.name}" must be "dictionary" with the "key" parameter`,
      )
    }
    const dictionary = parseStructuredField(
      combined,
      'dictionary',
      false,
      definition?.version === 'rfc9651',
    ) as SfDictionary
  ...
  if (sf) {
    ...
    return serializeStructuredField(
      parseStructuredField(combined, definition.type, false, definition.version === 'rfc9651'),
      definition.type,
    )
  }
 ...
 * do not treat the source request as independently readable afterward. Use {@link createSignature}
 ...
 * {@link createSignature} and construct the final message explicitly when both bodies must remain
 * readable.
 *
 ...
 * treat the source message as independently readable afterward. Use {@link createSignature} and
 ...
 * {@link createAcceptSignature} and construct the final message explicitly when both bodies must
 * remain readable.
 *
 * @group Signature Negotiation
 */
 ...
 *
 * Appending transfers or reuses the source body stream. Consume the returned message and do not
 * treat the source message as independently readable afterward. Use {@link createRequestedSignature}
 * and construct the final message explicitly when both bodies must remain readable.
 *
 * @group Signature Negotiation
 */
...
function snapshotStructuredFields(
  ...
  return Object.fromEntries(
    Object.entries(structuredFields).map(([name, definition]) => [
      name,
      definition !== null && typeof definition === 'object' ? { ...definition } : definition,
    ]),
  )
}
...
export function createSignedFetch(options: SignedFetchOptions): typeof globalThis.fetch {
  ...
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const signedRequest = await sign(request, signOptions)
    const response = await fetchImplementation(signedRequest)
