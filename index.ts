...
function assertUniqueComponents(components: ReadonlyArray<MessageComponent>): void {
  ...
  // number of covered components, which a peer chooses before any signature has been verified.
  ...
  for (const identifier of components) {
    ...
    if (typeof key === 'string') {
      // A Dictionary key is resolved from one field in one section, so `sf` and any other parameter
      // that does not select the value must not make an otherwise identical reference unique.
 ...
 * for.
 *
 * A covered component list may reference the same field many times, most often as a Dictionary with
 * a different `key` each time. Reading, combining, and parsing it once per reference would be
 * quadratic in the size of a field a peer chooses, so each derivation is computed on first use and
 * reused for the rest of the base.
 */
interface FieldDerivation {
  /** The individual field occurrences, after RFC 9421 field line canonicalization. */
  readonly values: ReadonlyArray<string>
  /** The occurrences combined with `", "`. */
  readonly combined: string
  /** Dictionary members indexed by key, built on first `key` use. */
  members?: Map<string, SfMember>
  /** The strict re-serialization, built on first `sf` use. */
  serialized?: string
}
...
/**
 * Rebuilds a request around new headers, carrying over the properties the Fetch constructor would
 * otherwise reset.
 *
 * The constructor resets an inherited referrer to `client` and an inherited referrer policy to the
 * empty string whenever `init` is not empty, so a caller that suppressed the `Referer` field would
 * silently get it back on the reconstructed request. Both are restored explicitly. The getters
 * report `""` for `no-referrer` and `"about:client"` for `client`, which the constructor accepts
 * and maps back to the same values.
 *
 * A `no-cors` request is rejected outright rather than rebuilt. Fetch gives such a request's
 * headers the `request-no-cors` guard, and none of the fields this package appends are
 * CORS-safelisted, so a browser would drop them and send an unsigned message while every operation
 * reported success. Node.js, Deno, and Bun do not enforce the guard, but a request that signs there
 * and silently does not in a browser is exactly the portability trap this package exists to avoid.
 */
function reconstructRequest(request: Request, headers: Headers, carried: string): Request {
 ...
 * on Cloudflare Workers, and `protocol`, `proxy`, `tls`, and `unix` on Bun, among others. The
 * wrappers turn the caller's initializer into a `Request` so it can be signed, and passing only
 ...
 * `referrerPolicy` are the exception, and are set from the signed request rather than the caller's
...
/**
 * Resolves with the signed request, or rejects as soon as the caller's signal aborts.
 *
 * A signer can be arbitrarily slow: an HSM, a remote signing service, or an application signer that
 * stalls. Awaiting it without observing the signal would leave the caller's `fetch()` pending for
 * as long as the signer takes, with no way to give up. When the abort wins, the transport is never
 * reached, because the wrapper never gets a request to send.
 */
function settleBeforeAbort<T>(
  operation: () => Promise<T>,
  signal: AbortSignal | null,
  onLoss?: (value: T | undefined) => void,
): Promise<T> {
  // The operation is started only once the signal has been checked, so an already-aborted request
  // never reaches a signer or a verifier and cannot trigger remote signing service or HSM work.
  if (signal?.aborted) {
  ...
  // The operation keeps running when it loses, so its result is consumed: an eventual rejection must
  // not surface as an unhandled rejection, and an eventual value may own a resource to release.
  const consume = () => {
    pending.then(
      (value) => onLoss?.(value),
      () => onLoss?.(undefined),
  ...
  // Starting the operation runs the signer or verifier factory synchronously, and that factory can
  // abort. An abort event is not replayed to a listener added afterwards, so the flag is read again
  // here rather than relying on the listener below to observe it.
  if (signal.aborted) {
    consume()
    return Promise.reject(signal.reason)
  }
...
/**
 * Releases the body of a message the caller will never receive.
 *
 * A wrapper that rejects keeps no reference the caller could use to cancel the stream itself, so an
 * unverified streaming response would hold its connection until garbage collection. Best effort:
 * the verification error is what matters, and a runtime that refuses the cancel must not replace
 * it.
 */
function cancelUndeliveredBody(message: Request | Response): void {
  try {
    const { body } = message
    // A body that has already been read is still cancellable as long as no reader holds the lock,
    // and cancelling it is what releases the underlying connection, so only a null or currently
    // locked body is skipped.
    if (body !== null && !body.locked) {
      // Deliberately not awaited. `cancel()` is allowed to reject, and nothing requires it to
      // settle at all, so awaiting it could hold back the error the caller is waiting for
      // indefinitely. Its rejection is consumed so it cannot surface as an unhandled rejection.
